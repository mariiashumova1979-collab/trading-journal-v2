<script lang="ts">
  ...
  function calc() {
    ...
    const compression = checkEventD1Compression(
      { high: d0.H, low: d0.L, volume: d0.V, atr14 },
      d1,
      candidate.direction
    );
    ...
    // Авто-заметка
    const noteLines = [
      `D1 (${d1Date}): ${compression.valid ? '✓ Compression OK' : '✗ Compression FAIL'}`,
      `OHLC O=— H=${d1.high.toFixed(2)} L=${d1.low.toFixed(2)} C=${d1.close.toFixed(2)} V=${d1.volume.toLocaleString()}`,
      `Vol D1/D0 = ${compression.volRatioD1D0.toFixed(2)} | InsideDay: ${compression.isInsideDay ? 'да' : 'нет'}`,
      ...(compression.errors.length ? ['Ошибки: ' + compression.errors.join('; ')] : [])
    ];
    if (compression.valid && !pos.invalidated) {
      noteLines.push(`Entry: $${pos.entry.toFixed(2)} | Stop: $${pos.stop.toFixed(2)} | Risk/ATR: ${pos.riskAtrRatio.toFixed(2)}`);
    }
  ...
  async function save() {
    ...
    try {
      ...
      if (result.status === 'READY_ENTRY' && !result.pos.invalidated) {
      ...
      // Если кандидат уже в сделке — добавить заметку
...
<div class="mo-bg" onclick={onClose} role="presentation">
  <div class="mo" onclick={(e) => e.stopPropagation()} role="dialog">
    <div class="mh">
      <div>D1 Compression — {candidate.ticker} ({candidate.direction})</div>
    ...
    <div class="info">
      ...
      <div>ATR14 = <b>{atr14.toFixed(2)}</b> · Midpoint D0 = <b>${midpoint.toFixed(2)}</b></div>
    </div>

    <div class="hint">
      <div><b>Compression LONG:</b> Vol D1 ≤ 0.7×D0 · Low D1 > Mid D0 · Close D1 > Mid D0</div>
      <div><b>Compression SHORT:</b> Vol D1 ≤ 0.7×D0 · High D1 &lt; Mid D0 · Close D1 &lt; Mid D0</div>
      <div><b>Entry D2:</b> {candidate.direction === 'LONG' ? 'High D1 + 0.1×ATR' : 'Low D1 − 0.1×ATR'} · <b>Stop:</b> {candidate.direction === 'LONG' ? 'Low D1 − 0.2×ATR' : 'High D1 + 0.2×ATR'}</div>
    </div>
    ...
    {#if result}
      ...
      <div class="prev" class:prev-ok={c.valid} class:prev-fail={!c.valid}>
        <div class="prev-h">{c.valid ? '✓ COMPRESSION OK' : '✗ COMPRESSION FAIL'}</div>
        ...
        {#if c.valid}
          <div style="margin-top:10px;padding-top:10px;border-top:1px solid var(--color-line)">
            {#if p.invalidated}
              <div style="color:var(--color-acc2)">⚠ {p.invalidReason} — вход не рекомендован</div>
            {:else}
              ...
              <div>T1 (+1R): <b>${p.target1.toFixed(2)}</b> · T2 (+3R): <b>${p.target2.toFixed(2)}</b></div>
            {/if}
          </div>
        {/if}

        <div class="note-prev">
          <div style="font-family:var(--font-mono);font-size:9px;color:var(--color-acc4);margin-bottom:4px">ЗАМЕТКА В СДЕЛКУ</div>
          <pre style="font-size:9px;color:var(--color-t2);margin:0;white-space:pre-wrap">{result.note}</pre>
        </div>
      </div>
    {/if}
    ...
    <div class="ar">
      ...
      <button onclick={save} disabled={!result || saving} class="btn-p">
        {saving ? 'Сохранение...' : 'Сохранить D1'}
      </button>
    </div>
  </div>
</div>
...
<style>
  ...
  .note-prev { margin-top: 10px; padding: 8px; background: rgba(0,0,0,0.2); border-radius: 4px; }
