<script lang="ts">
  ...
  const isGeneric = ['ibs_swing', 'nr7', 'event_continuation', 'pead', 'gap_reversal', 'atr_channel'].includes(candidate.strategy);
    ...
    try {
      ...
      const notes = isMaxWeekly && mwPayload
        ...
        : (() => {
            ...
            const trailRules = [
              '─ Управление (T0 = день входа) ─',
              'До T1: исходный стоп фиксирован, не двигаем',
              'T1 hit (+1R): закрыть 50-60%, стоп остатка → безубыток',
              candidate.direction === 'LONG'
                ? 'После T1 ежедневно: stop = max(stop, Low предыдущего дня)'
                : 'После T1 ежедневно: stop = min(stop, High предыдущего дня)',
              'T2 +2R · Time stop D+5 от входа'
            ].join('\n');
            const full = base + '\n' + trailRules;
