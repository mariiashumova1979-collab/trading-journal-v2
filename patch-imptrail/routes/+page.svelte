  ...
  function buildItems(cands: Candidate[], trds: Trade[]): TodayItem[] {
    ...
    for (const t of trds) {
      ...
      if (daysLeft <= 0) {
        ...
        timeLabel = 'После закрытия';
      } else if (s === 'impulse' && dayN >= 1 && t.partial_exit) {
        // Trailing ТОЛЬКО после T1 (partial_exit заполнен)
        when = 'evening';
        const curStop = t.stop != null ? Number(t.stop) : null;
        action = `D+${dayN} trail update (после T1)`;
        hint = dir === 'LONG'
          ? `Stop = max(текущий ${curStop != null ? '$'+curStop.toFixed(2) : '—'}, Low предыдущего дня) · только подтягиваем`
          : `Stop = min(текущий ${curStop != null ? '$'+curStop.toFixed(2) : '—'}, High предыдущего дня) · только подтягиваем`;
        timeLabel = 'После закрытия';
      } else if (s === 'impulse' && dayN >= 1) {
        // До T1 — стоп фиксирован, наблюдаем за T1
        when = 'watch';
        const t1 = t.target1 != null ? `$${Number(t.target1).toFixed(2)}` : '—';
        action = `D+${dayN} · до T1 стоп фиксирован`;
        hint = `Ждём T1 ${t1} (+1R) → закрыть 50-60%, стоп в BE, далее трейл`;
