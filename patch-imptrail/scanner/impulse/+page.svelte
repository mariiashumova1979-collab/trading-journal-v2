...
<div class="page">
  ...
  <div class="hint">
    ...
    <div style="color:var(--color-acc)"><b>Трейлинг ТОЛЬКО после T1:</b> до T1 исходный стоп фиксирован · T1 hit → закрыть 50-60%, стоп остатка в безубыток · после T1 ежедневно: LONG stop = max(stop, Low предыдущего дня), SHORT stop = min(stop, High предыдущего дня)</div>
  </div>
  ...
  <WorkflowGuide
    ...
    sections={[
      ...
      },
      {
        ...
        steps: [
          '**До T1**: исходный стоп фиксирован, НЕ двигается',
          '**T1 hit** (+1R): закрыть 50-60%, стоп остатка → **безубыток** (`target1`)',
          '**После T1 — ежедневный trailing** (на закрытии каждого дня):',
          '  • **LONG**: новый стоп = max(текущий стоп, Low предыдущего дня)',
          '  • **SHORT**: новый стоп = min(текущий стоп, High предыдущего дня)',
          'Стоп только подтягивается, никогда не отступает (exit при касании: `trail`)',
          'Пример (LONG после T1, стоп в BE $100): Low 101.20 → стоп 101.20 · след. день Low 100.10 → стоп **остаётся 101.20** · Low 103 → стоп 103',
